...
#define AVAILABLE_SCAN_COUNT(i)  ((i-1)*7)
...
void mgmt_disco_le_timeout(unsigned long data)
{
	...
	/* re-start BR scan */
		if (hdev->disco_state != SCAN_IDLE) {
			...
			cp.num_rsp = (u8) AVAILABLE_SCAN_COUNT(hdev->disco_int_phase);
			...
			u8 *dev_class, s8 rssi, u8 eir_len, u8 *eir)
{
	...
	if (hdev->disco_int_count >= AVAILABLE_SCAN_COUNT(hdev->disco_int_phase)) {
