...
void get_crash_time(unsigned char *buf, unsigned int bufsize)
{
...
void set_kcj_crash_info(void)
{
	...
	memcpy( &pPanicInfo->crash_system[0], &pSmemLogInfo->crash_system[0], CRASH_CODE_SIZE );
	memcpy( &pPanicInfo->crash_kind[0]  , &pSmemLogInfo->crash_kind[0]  , CRASH_CODE_SIZE );
...
static void resetlog_to_eMMC_kyocera_write( void )
{
    ...
    memcpy( &Emmc_log_info.rec_info[write_index].crash_system[0], pLogInfo->crash_system, CRASH_CODE_SIZE );
    memcpy( &Emmc_log_info.rec_info[write_index].crash_kind[0]  , pLogInfo->crash_kind  , CRASH_CODE_SIZE );
...
static void clear_kcjlog_info(void)
{
    memset( (void *)ADDR_CONTROL_INFO, 0x00, SIZE_CONTROL_INFO );
}
...
void dump_kcj_log( void )
{
    ...
    if ( (0 == strncmp((const char *)pPanicInfo->crash_system, CRASH_SYSTEM_KERNEL  , strlen(CRASH_SYSTEM_KERNEL ))) ||
         ...
         (0 == strncmp((const char *)pPanicInfo->crash_system, CRASH_SYSTEM_RIVA    , strlen(CRASH_SYSTEM_RIVA   ))) ||
         (0 == strncmp((const char *)pPanicInfo->crash_system, CRASH_SYSTEM_LPASS   , strlen(CRASH_SYSTEM_LPASS  ))) ||
         ...
         (0 == strncmp((const char *)pPanicInfo->crash_system, CRASH_SYSTEM_UNKNOWN , strlen(CRASH_SYSTEM_UNKNOWN))) )
    {
		...
		if (device_mem)
		{
			memcpy( (void *)ADDR_MODEM_F3_LOG, device_mem, pPanicInfo->Sizeof_Err_F3_Trace_Buffer );
			iounmap(device_mem);
		}
	    device_mem = ioremap_nocache(pPanicInfo->pErr_Data, pPanicInfo->Sizeof_Err_Data);
		if (device_mem)
		{
			memcpy( (void *)ADDR_MODEM_ERR_DATA_LOG, device_mem, pPanicInfo->Sizeof_Err_Data );
			iounmap(device_mem);
		}
	    device_mem = ioremap_nocache(pPanicInfo->pSmem_log_events, pPanicInfo->Sizeof_Smem_log);
		if (device_mem)
		{
			memcpy( (void *)ADDR_SMEM_EVENT_LOG, device_mem, pPanicInfo->Sizeof_Smem_log );
			iounmap(device_mem);
		}
