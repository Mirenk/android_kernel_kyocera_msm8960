...
static uint16_t guc_nv_proximity_sensor_near[10] = {0x03F0,0x03F0,0x03F0,0x03F0,0x03F0,
													0x03F0,0x03F0,0x03F0,0x03F0,0x03F0};
static uint16_t guc_nv_proximity_sensor_far[10]  = {0x03D0,0x03D0,0x03D0,0x03D0,0x03D0,
													0x03D0,0x03D0,0x03D0,0x03D0,0x03D0};
static uint16_t guc_nv_photo_sensor_beamish[1]  = {0xAAAA};
static uint16_t guc_nv_photo_sensor_dark[1]     = {0x9000};
static uint16_t guc_nv_photo_sensor_b[10]        = {0x07DA,0x07DA,0x07DA,0x07DA,0x07DA,
													0x07DA,0x07DA,0x07DA,0x07DA,0x07DA};
static uint16_t guc_nv_photo_sensor_c[10]        = {0x01E4,0x01E4,0x01E4,0x01E4,0x01E4,
													0x01E4,0x01E4,0x01E4,0x01E4,0x01E4};
static uint16_t guc_nv_photo_sensor_d[10]        = {0x038C,0x038C,0x038C,0x038C,0x038C,
													0x038C,0x038C,0x038C,0x038C,0x038C};
static uint16_t guc_nv_photo_sensor_ga[10]       = {0x0992,0x0992,0x0992,0x0992,0x0992,
													0x0992,0x0992,0x0992,0x0992,0x0992};
static uint16_t guc_nv_prox_photo_colvar[1]     = {0x0000};
static uint8_t guc_nv_proximity_sensor_ppcount[8][10] = {{0x04,0x04,0x04,0x04,0x04,0x02,0x02,0x02,0x02,0x02},
															{0x04,0x04,0x04,0x04,0x04,0x02,0x02,0x02,0x02,0x02},
															{0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02},
															{0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02},
															{0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02},
															{0x06,0x02,0x02,0x06,0x02,0x02,0x02,0x02,0x02,0x02},
															{0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02,0x02},
															{0x06,0x02,0x02,0x06,0x02,0x02,0x02,0x02,0x02,0x02}};
static uint32_t gun_nv_status                   = 0;
static atomic_t g_dev_status;
static oem_board_type g_board_type		= OEM_BOARD_MP_TYPE;
...
void apds990x_enable_ps_sensor(struct i2c_client *client)
{
...
#endif
	int colvar = guc_nv_prox_photo_colvar[0];
	int ppcount_val = guc_nv_proximity_sensor_ppcount[g_board_type][colvar];
	...
	if (data->enable_als_sensor<=0) {
		...
		apds990x_set_ppcount(client, ppcount_val);
...
static void apds990x_set_sensor_nv( unsigned long ulArg )
{
	...
	case en_NV_PROX_PHOTO_COLVAR_I:
		...
		break;
	case en_NV_PROXIMITY_SENSOR_PPCOUNT_I:
		APDS_DEBUG_LOG("%s en_NV_PROXIMITY_SENSOR_PPCOUNT_I\n",__func__);
		gun_nv_status |= (0x01<<en_NV_PROXIMITY_SENSOR_PPCOUNT_I);
		memcpy(guc_nv_proximity_sensor_ppcount,
				nv_data_type->ucData,(size_t)nv_data_type->ulLength);
		break;
	default :
                   ...
                   const struct i2c_device_id *id)
{
	...
	g_board_type = OEM_get_board();
	APDS_DEBUG_LOG("g_board_type = %d\n", g_board_type);
