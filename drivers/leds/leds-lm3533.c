...
#define LM3533_MOBILE_OUT_GPIO	26
#define LM3533_MOBILE_IN_GPIO	27
...
#define LM3553_CURRENT_VALUE_MIN_RED	0x70
#define LM3553_CURRENT_VALUE_MIN_GREEN	0x04
#define LM3553_CURRENT_VALUE_MIN_BLUE	0x8B
#define LM3553_CURRENT_VALUE_OFF		0x00
#define LM3553_CURRENT_VALUE_RED		0x90
#define LM3553_CURRENT_VALUE_GREEN		0x89
#define LM3553_CURRENT_VALUE_BLUE		0xD0
...
#define MOBILELIGHT_BATTTERM_LOW_TORCH_VAL		0x13
#define MOBILELIGHT_BATTTERM_HIGH_VAL			0x05
#define MOBILELIGHT_BATTTERM_HIGH_TORCH_VAL		0x05
#define MOBILELIGHT_IN_BATTTERM_LOW_VAL			0x13
#define MOBILELIGHT_IN_BATTTERM_LOW_TORCH_VAL	0x13
#define MOBILELIGHT_IN_BATTTERM_HIGH_VAL		0x05
#define MOBILELIGHT_IN_BATTTERM_HIGH_TORCH_VAL	0x05
...
enum mobilelight_type_enum{
	MOBILELIGHT_NONE = 0,
	MOBILELIGHT_OUT,
	MOBILELIGHT_IN
};

enum mobilelight_flash_mode_enum{
	MOBILELIGHT_NONE_MODE =0,
	MOBILELIGHT_FLASH_MODE,
	MOBILELIGHT_TORCH_MODE
};

static uint32_t const gul_pattern_val[COLOR_PATTERN_MAX][LED_COLOR_PATTERN_RGB] = {
	{LM3553_CURRENT_VALUE_OFF     ,LM3553_CURRENT_VALUE_OFF       ,LM3553_CURRENT_VALUE_BLUE     },
	{LM3553_CURRENT_VALUE_RED     ,LM3553_CURRENT_VALUE_OFF       ,LM3553_CURRENT_VALUE_OFF      },
	{LM3553_CURRENT_VALUE_OFF     ,LM3553_CURRENT_VALUE_GREEN     ,LM3553_CURRENT_VALUE_OFF      },
	{LM3553_CURRENT_VALUE_RED     ,LM3553_CURRENT_VALUE_OFF       ,LM3553_CURRENT_VALUE_MIN_BLUE },
	{LM3553_CURRENT_VALUE_OFF     ,LM3553_CURRENT_VALUE_GREEN     ,LM3553_CURRENT_VALUE_MIN_BLUE },
	{LM3553_CURRENT_VALUE_RED     ,LM3553_CURRENT_VALUE_GREEN     ,LM3553_CURRENT_VALUE_OFF      },
	{LM3553_CURRENT_VALUE_MIN_RED ,LM3553_CURRENT_VALUE_GREEN     ,LM3553_CURRENT_VALUE_MIN_BLUE }
};
...
static uint32_t guc_mobilelight_save_val = MOBILELIGHT_NONE_MODE;
static uint32_t gul_value = 0;

static uint32_t gul_mobilelight_type = MOBILELIGHT_NONE;
...
static void mobilelight_set(struct light_led_data_type *pst_light_led_data)
{
	...
	int32_t batt_temp = 0;
	int32_t mobilelight_inout = MOBILELIGHT_OUT;
	int32_t mobilelight_flash_mode = MOBILELIGHT_FLASH_MODE;

	DEBUG_PRINT("%s() start \n", __func__);

	if(pst_light_led_data->ul_value&0x01000000){
		mobilelight_inout = MOBILELIGHT_IN;
	}

	if(MOBILELIGHT_TORCH_MODE == (pst_light_led_data->ul_value&0x00FFFFFF)){
		mobilelight_flash_mode = MOBILELIGHT_TORCH_MODE;
	}

	DEBUG_PRINT("%s() mobilelight_inout = %d mobilelight_flash_mode=%d\n", __func__,mobilelight_inout,mobilelight_flash_mode);
	...
	if (LM3533_COL_BLACK == (pst_light_led_data->ul_value&0x00FFFFFF)) {
		...
		gpio_set_value_cansleep(LM3533_MOBILE_OUT_GPIO, 0);
		gpio_set_value_cansleep(LM3533_MOBILE_IN_GPIO, 0);
		gul_mobilelight_type = MOBILELIGHT_NONE;
		guc_mobilelight_save_val = MOBILELIGHT_NONE_MODE;
	}
	else {
		if(mobilelight_inout != gul_mobilelight_type){
			if(gul_mobilelight_type != MOBILELIGHT_NONE)
			{
				DEBUG_PRINT("%s() gul_mobilelight_type = %d \n", __func__,gul_mobilelight_type);
				lret = lm3533_i2c_write(pst_light_led_data->pst_client,
							LM3533_REG_27,
							(guc_light_status&(~MOBILE_LIGHT_STATE)));
				if (lret != 0) {
                        		DEBUG_PRINT_ERR("%s() failed to mobilelight in/out chg state = %d \n",
						__func__,
						guc_light_status);
					goto fail_i2c_check;
				}
			}
			if(mobilelight_inout == MOBILELIGHT_OUT){
				DEBUG_PRINT("%s() MOBILELIGHT_OUT \n", __func__);
				gpio_set_value_cansleep(LM3533_MOBILE_IN_GPIO, 0);
				gpio_set_value_cansleep(LM3533_MOBILE_OUT_GPIO, 1);
				gul_mobilelight_type = MOBILELIGHT_OUT;
			}
			else{
				DEBUG_PRINT("%s() MOBILELIGHT_IN \n", __func__);
				gpio_set_value_cansleep(LM3533_MOBILE_OUT_GPIO, 0);
				gpio_set_value_cansleep(LM3533_MOBILE_IN_GPIO, 1);
				gul_mobilelight_type = MOBILELIGHT_IN;
			}
		}
		
		guc_mobilelight_save_val = (pst_light_led_data->ul_value&0x00FFFFFF);
		DEBUG_PRINT("%s() guc_mobilelight_save_val = %x\n",__func__,guc_mobilelight_save_val);
		...
		if (guc_light_dm == 1) {
			if(mobilelight_inout == MOBILELIGHT_OUT){
				uc_mobilelight_val = (mobilelight_flash_mode == MOBILELIGHT_TORCH_MODE)?
							MOBILELIGHT_BATTTERM_HIGH_TORCH_VAL:MOBILELIGHT_BATTTERM_HIGH_VAL;
			}
			else{
				uc_mobilelight_val = (mobilelight_flash_mode == MOBILELIGHT_TORCH_MODE)?
							MOBILELIGHT_IN_BATTTERM_HIGH_TORCH_VAL:MOBILELIGHT_IN_BATTTERM_HIGH_VAL;
			}
			...
			if(mobilelight_inout == MOBILELIGHT_OUT){
				uc_mobilelight_val = (mobilelight_flash_mode == MOBILELIGHT_TORCH_MODE)?
							MOBILELIGHT_BATTTERM_LOW_TORCH_VAL:MOBILELIGHT_BATTTERM_LOW_VAL;
			}
			else{
				uc_mobilelight_val = (mobilelight_flash_mode == MOBILELIGHT_TORCH_MODE)?
							MOBILELIGHT_IN_BATTTERM_LOW_TORCH_VAL:MOBILELIGHT_IN_BATTTERM_LOW_VAL;
			}
			DEBUG_PRINT("%s() mobilelight_val = %d\n", 
						__func__,
						uc_mobilelight_val);
...
static int32_t light_led_err_reset(struct light_led_data_type *pst_light_led_data)
{
	...
	if ((guc_light_status & MOBILE_LIGHT_STATE) != 0) {
		...
		pst_light_led_data->ul_value = 
			(guc_mobilelight_save_val | ((gul_mobilelight_type == MOBILELIGHT_IN)?(1<<24):0));
		mobilelight_set(pst_light_led_data);
	}
